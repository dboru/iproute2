...
{
	fprintf(stderr, "Usage: ... dualpi2 [limit PACKETS] [target TIME]");
	fprintf(stderr, "		[tupdate TIME]\n");
	fprintf(stderr, "               [alpha ALPHA] [beta BETA]\n");
	fprintf(stderr, "               [no_dualq|l4s_dualq|dc_dualq]");
	fprintf(stderr,	"		[k KFACTOR]\n");
	fprintf(stderr, "               [no_ecn|classic_ecn|l4s_ecn|dc_ecn]\n");
	fprintf(stderr, "               [et_packets|et_time] ");
	fprintf(stderr, "		[l_thresh TIME|PACKETS]");
	fprintf(stderr, "		[t_shift TIME]\n");
...
#endif
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "limit") == 0) {
			...
			if (get_float(&alpha_f, *argv) ||
			    alpha_f > ALPHA_BETA_MAX ||
			    alpha_f < ALPHA_BETA_MIN) {
				fprintf(stderr, "Illegal \"alpha\"\n");
				return -1;
			}
			if (alpha_f == 0) {
				fprintf(stderr, "Warning: \"alpha\" is zero.");
				fprintf(stderr, " Integral controller will");
				fprintf(stderr, " be disabled.\n");
			} else if (alpha_f < ALPHA_BETA_MIN_ENABLED) {
				fprintf(stderr, "Warning: \"alpha\" is too ");
				fprintf(stderr, "small and will be rounded ");
				fprintf(stderr, "to zero. Integral controller");
			...
			if (get_float(&beta_f, *argv) ||
			    beta_f > ALPHA_BETA_MAX ||
			    beta_f < ALPHA_BETA_MIN) {
				fprintf(stderr, "Illegal \"beta\"\n");
				return -1;
			}
			if (beta_f == 0) {
				fprintf(stderr, "Warning: \"beta\" is zero. ");
				fprintf(stderr, "Proportional controller will");
				fprintf(stderr, "be disabled.\n");
			} else if (beta_f < ALPHA_BETA_MIN_ENABLED) {
				fprintf(stderr, "Warning: \"beta\" is too ");
				fprintf(stderr, "small and will be rounded to");
				fprintf(stderr, " zero. Proportional ");
				fprintf(stderr, "controller will be");
	...
	if (c_limit && l_drop) {
		fprintf(stderr, "c_limit cannot be used with l_drop, use ");
		fprintf(stderr, "either c_limit or l_drop (refer to README)");
		explain();
		return -1;
	}

	if (et_packets && et_time) {
		fprintf(stderr, "et_packets cannot be used with et_time, use ");
		fprintf(stderr, "either et_packets or et_time )");
	...
	if (tupdate)
		addattr_l(n, 1024, TCA_DUALPI2_TUPDATE, &tupdate,
			  sizeof(tupdate));
	if (target)
	...
	if (queue_mask != -1)
		addattr_l(n, 1024, TCA_DUALPI2_DUALQ, &queue_mask,
			  sizeof(queue_mask));
	if (ecn != -1)
	...
	if (et_packets)
		addattr_l(n, 1024, TCA_DUALPI2_ET_PACKETS, &et_packets,
			  sizeof(et_packets));
	if (l_thresh)
		addattr_l(n, 1024, TCA_DUALPI2_L_THRESH, &l_thresh,
			  sizeof(l_thresh));
	if (t_shift)
		addattr_l(n, 1024, TCA_DUALPI2_T_SHIFT, &t_shift,
			  sizeof(t_shift));
	if (t_speed)
		addattr_l(n, 1024, TCA_DUALPI2_T_SPEED, &t_speed,
			  sizeof(t_speed));
	if (drop_early != -1)
		addattr_l(n, 1024, TCA_DUALPI2_DROP_EARLY, &drop_early,
			  sizeof(drop_early));
	addattr_l(n, 1024, TCA_DUALPI2_WRR_RATIO, &wrr_ratio,
		  sizeof(wrr_ratio));
			    ...
			    struct rtattr *xstats)
{
	...
	fprintf(f, "prob %f delay_c %uus delay_l %uus\n", 
		(double)st->prob / (double)0xffffffff, st->delay_c,
		st->delay_l);
